...
namespace CombatHandler.Generic
{
    public class GenericCombatHandler : AOSharp.Core.Combat.CombatHandler
    {
        ...
        public GenericCombatHandler()
        {
            ...
            RegisterPerkProcessor(PerkHash.Awakening, TeamHealPerk);
            ...
            Chat.RegisterCommand("stack".ToLower(), StackCommand);
            Chat.RegisterCommand("dupe".ToLower(), DupeCommand);
        ...
        private void DupeCommand(string command, string[] param, ChatWindow chatWindow)
        {
            try
            {
                ...
                if(param.Length > 0)
                {
                    Chat.WriteLine("Invalid command ", ChatColor.Green);
                    return;
                }
        ...
        private void StackCommand(string command, string[] param, ChatWindow chatWindow)
        {
            ...
            if (param.Length < 1)
            {
                ...
                Chat.WriteLine("Invalid command");
                return;
            }
            if (param.Length == 1)
            {
                switch (param[0].ToLower())
                {
                        ...
                        try
                        {
                            ...
                            foreach (Item item in items)
                            {
                                ...
                                if (item.Name == "Combined Commando's Gloves" || item.Name == "Combined Commando's Jacket" || item.Name == "Combined Commando's Sleeves" || item.Name == "Combined Commando's Legs")
                                {
                                    ...
                                    {
                                        Chat.WriteLine(" Found  " + item.Name + " " + item.LowId + " ql " + item.QualityLevel);
                                    }
                                    else
                                    {
                                        Chat.WriteLine("  No items");
                                        return;
                                    }
                                }
                            }
                    ...
                    case "o":
                        stackEnabled = false;
                        Chat.WriteLine("Stack command disabled", ChatColor.Green);
                        break;
