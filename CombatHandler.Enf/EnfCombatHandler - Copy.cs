...
namespace Desu
{
    class EnfCombatHandler : GenericCombatHandler
    {
        ...
        public EnfCombatHandler()
        {
            ...
            RegisterPerkProcessor(PerkHash.QuickBash, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.CrushBone, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.BringThePain, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.DevastatingBlow, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.BigSmash, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.FollowupSmash, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.SeismicSmash, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.DisableNaturalHealing, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.Taunt, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.Charge, DamagePerk);
            RegisterPerkProcessor(PerkHash.Headbutt, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.GroinKick, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.ArouseAnger, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.CauseOfAnger, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.Highway, TargetedDamagePerk);
            RegisterPerkProcessor(PerkHash.TrollForm, TargetedDamagePerk);
            ...
            RegisterPerkProcessor(PerkHash.LEProcEnforcerVortexOfHate, SelfBuffPerk, CombatActionPriority.Medium);//Type1
            RegisterPerkProcessor(PerkHash.LEProcEnforcerViolationBuffer, SelfBuffPerk, CombatActionPriority.Low);//Type2
            ...
            RegisterSpellProcessor(Spell.GetSpellsForNanoline(NanoLine.EnforcerPiercingBuff).OrderByStackingOrder(), GenericBuff);
            RegisterSpellProcessor(Spell.GetSpellsForNanoline(NanoLine.EnforcerTauntProcsDreadbringer).OrderByStackingOrder(), GenericBuff);
            RegisterSpellProcessor(Spell.GetSpellsForNanoline(NanoLine.AbsorbACBuff).OrderByStackingOrder(), GenericBuff);
            ...
            RegisterSpellProcessor(Spell.GetSpellsForNanoline(NanoLine.DamageChangeBuffs).OrderByStackingOrder(), GenericBuff);
            RegisterSpellProcessor(RelevantNanos.ELEMENT_OF_MALICE, SingleTargetTaunt, CombatActionPriority.High);
            RegisterSpellProcessor(RelevantNanos.CallingofThor, SingleTargetTaunt);
            RegisterSpellProcessor(RelevantNanos.Ibehe, TeamBuff);
        ...
        private static class RelevantNanos
        {
           ...
           public const int Mongo = 270786;
           public const int CoruscatingScreen = 55751;
           public const int Ibehe = 273629;
        }
    }
}
